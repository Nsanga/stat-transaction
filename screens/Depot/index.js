  ...
  const receive = () => {
    ...
    db.transaction(
      (tx) => {
        ...
        try {
          tx.executeSql("insert into operation (operateur,telephone,type, montant,idGerant,dateTransaction,heuretransaction ) values (?,?,?, ?,?,?,?)", [titre, tel,'depot', amount, user,date,heure]);
